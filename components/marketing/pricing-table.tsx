...
const tierAccents = [
  "from-white/[0.1] via-white/[0.05] to-transparent",
  ...
  return (
    ...
    >
      <PageShell>
        ...
        <div className={`grid gap-5 lg:grid-cols-3 ${showHeading ? "mt-14" : "mt-0"}`}>
          {pricingTiers.map((tier, index) => (
            ...
            >
              ...
              <div className={`pointer-events-none absolute inset-x-8 top-0 h-28 bg-gradient-to-b ${tierAccents[index]} blur-2xl transition duration-500 group-hover:opacity-100 ${index === 1 ? "opacity-100" : "opacity-60"}`} />
              <div className="relative flex items-center justify-between gap-4">
                <h3 className="text-2xl font-semibold leading-tight text-white">{tier.name}</h3>
                <span className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${index === 1 ? "border-white/[0.2] bg-white text-ink" : "border-white/[0.12] bg-white/[0.07] text-white/70"}`}>
                  {tier.badge}
                </span>
              </div>
              ...
              <ul className="relative mt-8 flex-1 space-y-4">
                {tier.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-3 text-sm text-white/[0.78]">
                    <span className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${index === 1 ? "bg-white text-ink" : "bg-white/[0.08] text-white"}`}>
              ...
              <Button href="/contact" variant={index === 1 ? "primary" : "secondary"} className="relative mt-9 w-full justify-center py-3.5">
