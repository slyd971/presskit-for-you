...
export function CategoryContent({ category }: CategoryContentProps) {
  return (
    <>
          ...
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-7">
            ...
            <ul className="mt-6 space-y-4">
              {category.includes.map((item) => (
                <li key={item} className="rounded-[1.2rem] border border-white/8 bg-white/[0.03] px-4 py-3 text-sm text-white/76">
                  {item}
                </li>
              ))}
            </ul>
          ...
          <div className="rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.07),rgba(255,255,255,0.03))] p-7">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              {category.modules.map((module) => (
                <div key={module} className="rounded-[1.4rem] border border-white/8 bg-[#0d1016] p-5">
      ...
      <section className="py-20">
        <PageShell>
          <div className="rounded-[2.1rem] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.12),transparent_20%),linear-gradient(180deg,rgba(255,255,255,0.07),rgba(255,255,255,0.03))] p-8 md:flex md:items-end md:justify-between md:gap-10">
